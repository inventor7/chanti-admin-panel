...
const selectedValues = computed(() => new Set<string>(props.column?.getFilterValue() as string[]))
        ...
        <CommandList>
          ...
          <CommandGroup>
            <CommandItem v-for="option in options" :key="option.value" :value="option" @select="(e) => {
              ...
              <div class=" space-x-2"> <span v-if=" title === 'State'" >{{ option.value }}</span> <span>{{ option.label }}</span></div>
